...
$domains = $template->get_template_vars('domains');
$i = 0;
foreach ($domains as $value) 
{
    if (is_array($value))
    {
        ...
        $pattern = '/(\.com\.hr|\.hr)/';
        $domain['flag'] = preg_match($pattern, $domain['domain']);
        if($domain['flag'])
        {
            $domains[$i]['contact_modify_link'] = "https://korisnik.infonet.hr/clientarea.php?action=domaincontacts&domainid=" . $domains[$i]['id'];
        }
        else 
        {
            $domains[$i]['contact_modify_link'] = "https://korisnik.infonet.hr/domainmanagement.php?action=domaincontacts&domainid=" . $domains[$i]['id'] . "&domain=" . $domains[$i]['domain'];
        }
        $domains[$i]['flag'] = $domain['flag'];
        $i++;
    }
} 
                                ...
                                <div class="btn-group btn-group-sm">
                                    ...
                                    <ul class="dropdown-menu text-left" role="menu">
                                        {if $domain.status eq 'Active'}
                                            ...
                                            <li><a href="{$domain.contact_modify_link}"><i class="glyphicon glyphicon-user"></i> {$LANG.domaincontactinfoedit}</a></li>
